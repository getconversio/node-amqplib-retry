...
return-1}return delay*1000};module.exports=function(channel,clientQueueName,failureQueueName,clientHandler,delayFunction,initializer){var errorHandler=function errorHandler(msg,err){if(!initializer.isInitialized){// Delay in 1 MS to let the queues/exchange/bindings initialize
...
msg.properties.headers._retryLastError=String(err&&err.message).substring(0,255);msg.properties.headers._retryCount+=1;var expiration=(delayFunction||getDefaultDelay)(msg.properties.headers._retryCount);if(expiration<1){return channel.sendToQueue(failureQueueName,new Buffer(msg.content),msg.properties)}var properties={persistent:true,headers:{_originalProperties:msg.properties,// save the original properties.
...
console.error('Error: AMQP retry handler caught the following error: ',err);return Promise.try(function(){return errorHandler(msg,err)}).catch(function(err){// Something went wrong while trying to process the erroneous message.
